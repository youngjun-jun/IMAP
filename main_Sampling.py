...
def load_model(model_id: str, device: str):
    ...
    if "CogVideoX" in model_path:
        ...
        from imap.cogvideox.pipeline import ModifiedCogVideoXPipeline, RenoiseCogVideoXPipeline
        ...
        from imap.hunyuanvideo.pipeline import ModifiedHunyuanVideoPipeline, RenoiseHunyuanVideoPipeline
        ...
        pipe = ModifiedHunyuanVideoPipeline.from_pretrained(
            model_path, 
            transformer=transformer,
            torch_dtype=dtype
        ).to(device)
        ...
        from imap.wan.pipeline import ModifiedWanPipeline, RenoiseWanPipeline
        ...
        pipe = ModifiedWanPipeline.from_pretrained(
            model_path, 
            transformer=transformer,
            vae=vae, 
            torch_dtype=dtype
...
def main(args):
    ...
    for i, input_dict in enumerate(input_dicts):
        ...
        # 2. check if output video already exists
